...
from AgroBusiness.views import ProductListView, otp_verify_view, check_image_ai, save_product, AddProduct, ProductDetailView
...
urlpatterns = [
    ...
    path('agro-add/', AddProduct.as_view(), name='agroadd'),
    path('product/<slug:slug>', ProductDetailView.as_view(), name='detail'),
    path('agro-add-product/', save_product, name='agro_add'),
    path('agro-otp/', otp_verify_view, name='agro_otp'),
    path('check-image/', check_image_ai, name='check_img'),
